<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>FurEver Care</title>
  <link rel="stylesheet" href="css/styles.css">
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>

  <!-- navbar to beh -->
<?php include 'partials/navbar.php'; ?>

  <!-- Adoption Section -->
  <section id="adoption" class="py-5">
    <div class="container">
      <div class="row align-items-center">
        
        <!-- Adoption Info -->
        <div class="col-lg-6 mb-4">
          <h2 class="fw-bold mb-3">Adopt a Shelter Cat or Dog</h2>
          <p class="mb-4">
            All of our pets are spayed/neutered (<em>kapon</em>) and vaccinated.  
            Since many have come from difficult lives, we want to ensure they find safe, loving homes.  
            Here’s how to adopt:
          </p>
          <ul class="list-unstyled mb-4">
            <li>📄 Submit the adoption application</li>
            <li>🎥 Attend the Zoom interview</li>
            <li>🐾 Meet our shelter animals in person</li>
            <li>💛 Visit and confirm your chosen pet</li>
            <li>🩺 Wait for vet clearance</li>
            <li>💵 Pay adoption fee: <strong>₱500 (cat) / ₱1000 (dog)</strong></li>
            <li>🏡 Take your new best friend home!</li>
          </ul>
          <div class="d-flex gap-3">
            <a href="apply.php" class="btn btn-dark">Apply Now</a>
            <a href="#" class="btn btn-outline-dark">Adoption FAQ</a>
          </div>
        </div>

        <!-- Pets Showcase -->
        <div class="col-lg-6">
          <div class="row g-3">
            <div class="col-md-4 col-6">
              <div class="card shadow-sm h-100 text-center">
                <img src="assets/img/adopt2.png" class="card-img-top" alt="Cat Owen">
                <div class="card-body p-2">
                  <h6 class="fw-bold text-success mb-0">OWEN</h6>
                </div>
              </div>
            </div>
            <div class="col-md-4 col-6">
              <div class="card shadow-sm h-100 text-center">
                <img src="assets/img/adopt3.png" class="card-img-top" alt="Dog Tiamo">
                <div class="card-body p-2">
                  <h6 class="fw-bold text-success mb-0">TIAMO</h6>
                </div>
              </div>
            </div>
            <div class="col-md-4 col-6">
              <div class="card shadow-sm h-100 text-center">
                <img src="assets/img/adopt4.png" class="card-img-top" alt="Dog Agie">
                <div class="card-body p-2">
                  <h6 class="fw-bold text-success mb-0">AGIE</h6>
                </div>
              </div>
            </div>
          </div>
        </div>

      </div>
    </div>
  </section>

  <section id="pet-gallery" class="py-5 bg-light">
    <div class="container">
      <div class="text-center mb-5">
        <h2 class="fw-bold">Meet Our Adoptable Pets</h2>
        <p class="text-muted">Find your new best friend! All our pets are waiting for a loving home.</p>
      </div>

      <div id="pet-filters" class="d-flex justify-content-center gap-2 mb-4">
        <button class="btn btn-dark active" data-filter="all">All Pets</button>
        <button class="btn btn-outline-dark" data-filter="dog">Dogs <i class="bi bi-heart-fill"></i></button>
        <button class="btn btn-outline-dark" data-filter="cat">Cats <i class="bi bi-heart"></i></button>
      </div>

      <div class="row g-4" id="pet-list">
        
        <div class="col-lg-3 col-md-4 col-6 pet-card" data-type="dog">
          <div class="card h-100 shadow-sm">
            <img src="https://images.unsplash.com/photo-1583512603805-3cc6b41f3edb?q=80&w=2832&auto=format&fit=crop" class="card-img-top" alt="A happy dog">
            <div class="card-body text-center d-flex flex-column">
              <h5 class="card-title fw-bold">Buddy</h5>
              <p class="card-text small text-muted mb-auto">2 y/o | Golden Retriever</p>
              <a href="apply.php?pet=Buddy" class="btn btn-sm btn-primary mt-3">Adopt Me</a>
            </div>
          </div>
        </div>

        <div class="col-lg-3 col-md-4 col-6 pet-card" data-type="cat">
          <div class="card h-100 shadow-sm">
            <img src="https://images.unsplash.com/photo-1574158622682-e40e6984100d?q=80&w=2933&auto=format&fit=crop" class="card-img-top" alt="A playful cat">
            <div class="card-body text-center d-flex flex-column">
              <h5 class="card-title fw-bold">Whiskers</h5>
              <p class="card-text small text-muted mb-auto">1 y/o | Domestic Shorthair</p>
              <a href="apply.php?pet=Whiskers" class="btn btn-sm btn-primary mt-3">Adopt Me</a>
            </div>
          </div>
        </div>
        
        <div class="col-lg-3 col-md-4 col-6 pet-card" data-type="dog">
          <div class="card h-100 shadow-sm">
            <img src="https://images.unsplash.com/photo-1598133894008-61f7fdb8cc3a?q=80&w=2787&auto=format&fit=crop" class="card-img-top" alt="A small friendly dog">
            <div class="card-body text-center d-flex flex-column">
              <h5 class="card-title fw-bold">Max</h5>
              <p class="card-text small text-muted mb-auto">3 y/o | Beagle</p>
              <a href="apply.php?pet=Max" class="btn btn-sm btn-primary mt-3">Adopt Me</a>
            </div>
          </div>
        </div>

        <div class="col-lg-3 col-md-4 col-6 pet-card" data-type="cat">
          <div class="card h-100 shadow-sm">
            <img src="https://images.unsplash.com/photo-1592194991823-23ae48953097?q=80&w=2787&auto=format&fit=crop" class="card-img-top" alt="A fluffy white cat">
            <div class="card-body text-center d-flex flex-column">
              <h5 class="card-title fw-bold">Luna</h5>
              <p class="card-text small text-muted mb-auto">2 y/o | Persian Mix</p>
              <a href="apply.php?pet=Luna" class="btn btn-sm btn-primary mt-3">Adopt Me</a>
            </div>
          </div>
        </div>
        
        </div>
    </div>
  </section>

  <section id="testimonials" class="py-5 bg-light">
      <div class="container">
        <div class="text-center mb-5">
          <h2 class="fw-bold">Happy Paws, Happy Parents</h2>
          <p class="text-muted">Here's what our clients have to say about their FurEver Care experience.</p>
        </div>
        <div id="testimonialCarousel" class="carousel slide" data-bs-ride="carousel">
          <div class="carousel-inner">
            <div class="carousel-item active">
              <div class="testimonial-card p-4 col-lg-8 mx-auto">
                <blockquote class="blockquote mb-0">
                  <p>"The team at FurEver Care is amazing. They treated our dog, Max, with so much love and expertise during his surgery. We couldn't be more grateful!"</p>
                  <footer class="blockquote-footer mt-3 d-flex align-items-center testimonial-author">
                    <img src="https://i.pravatar.cc/150?u=a042581f4e29026704d" class="rounded-circle me-3" alt="Client photo">
                    <span>Maria Dela Cruz, with <cite title="Source Title">Max</cite></span>
                  </footer>
                </blockquote>
              </div>
            </div>
            <div class="carousel-item">
              <div class="testimonial-card p-4 col-lg-8 mx-auto">
                <blockquote class="blockquote mb-0">
                  <p>"We adopted our cat, Luna, from here and the process was so smooth and thorough. They truly care about finding the right homes for their animals."</p>
                  <footer class="blockquote-footer mt-3 d-flex align-items-center testimonial-author">
                    <img src="https://i.pravatar.cc/150?u=a042581f4e29026705d" class="rounded-circle me-3" alt="Client photo">
                    <span>John Santos, with <cite title="Source Title">Luna</cite></span>
                  </footer>
                </blockquote>
              </div>
            </div>
          </div>
          <button class="carousel-control-prev" type="button" data-bs-target="#testimonialCarousel" data-bs-slide="prev">
            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
            <span class="visually-hidden">Previous</span>
          </button>
          <button class="carousel-control-next" type="button" data-bs-target="#testimonialCarousel" data-bs-slide="next">
            <span class="carousel-control-next-icon" aria-hidden="true"></span>
            <span class="visually-hidden">Next</span>
          </button>
        </div>
      </div>
  </section>

  <!-- footer -->
  <footer class="site-footer">
    <div class="footer-container">
      <p>&copy; 2025 Furever Care. All Rights Reserved.</p>
      <nav class="footer-nav">
        <a href="#services">Services</a>
        <a href="#why">Why Choose Us</a>
        <a href="#adoption">Adopt</a>
        <a href="#comments">Comments</a>
      </nav>
    </div>
  </footer>


  <!-- Floating Contact Us -->
<a href="#" class="contact-btn" onclick="openContactPanel()">Contact Us</a>

<div id="contact-panel" class="contact-panel">
  <div class="panel-header">
    <h2>Contact Us</h2>
    <span class="close-btn" onclick="closeContactPanel()">&times;</span>
  </div>
  <form class="panel-form" id="contactForm">
    <input type="text" placeholder="Your Name" required>
    <input type="email" placeholder="Your Email" required>
    <textarea rows="4" placeholder="Your Message" required></textarea>
    <button type="submit">Send Message</button>
  </form>
  <div class="panel-socials">
    <p>Or reach us at:</p>
    <a href="#">Email</a> |
    <a href="#">Call</a>
  </div>
</div>

<script>
function openContactPanel() {
  document.getElementById("contact-panel").classList.add("open");
}

function closeContactPanel() {
  document.getElementById("contact-panel").classList.remove("open");
}

// Handle form submission
document.getElementById("contactForm").addEventListener("submit", function(event) {
  event.preventDefault(); 
  alert("Your message has been sent! 🐾");
  this.reset();
  closeContactPanel();
});
</script>

  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>
